...
void amics() {
    ...
    char nom[LONGITUD];
    int i, anyActual, mesActual, diaActual, error, editar;
    FILE *fp;

    printf("%s", linies[31]);
    scanf("%s", colla.nomColla);

    snprintf(nom, LONGITUD, "%s.dat", colla.nomColla);
    ...
    printf("%s", linies[39]);		
	scanf("%d", &editar);
	if(editar==CERT)	editarAmics();  //PREGUNTAR SI QUIEREN EDITAR
...
void comprovarPremiPetitColla (int amic) {
	...
	//APROXIMACIONS (1r, 2n, 3r)
        ...
        if(colla.persona[amic].premi==0) printf("%s", linies[27]);
}
//--------------------------------------------------------------
...
//--------------------------------------------------------------
void editarAmics () {
	int opcio=0, trobat=0, i, j;
	char user[LONGITUD], nom[LONGITUD];
	FILE *fp;
	
	do {
		printf("%s \n %s %s %s %s \n%s", linies[40], linies[41], linies[42], linies[43], linies[44], linies[50]);
		scanf("%d", &opcio);
		switch(opcio){
			case 0:
				...
				do {
					...
					scanf("%s", user);
						for (i = 0; i < colla.numPersones; i++) {
							if (strcmp(user, colla.persona[i].nom) == 0) {
								trobat = 1;
								for(j = i; j < colla.numPersones-1; j++) colla.persona[j] = colla.persona[j+1];
								colla.numPersones--;				
							}
						}
					if (!trobat) printf("%s", linies[47]);
				} while (!trobat);
				break;	
			...
			case 1:
				...
				colla.numPersones++;
				printf("%s", linies[36]);
				scanf("%s", colla.persona[colla.numPersones-1].nom);
				printf("%s", linies[37]);
				do {
					scanf("%d", &colla.persona[colla.numPersones-1].nDecim);
					if (colla.persona[colla.numPersones-1].nDecim < 0 || colla.persona[colla.numPersones-1].nDecim > 99999)
					printf("%s", linies[3]);
				} while (colla.persona[colla.numPersones-1].nDecim < 0 || colla.persona[colla.numPersones-1].nDecim > 99999);
				printf("%s", linies[38]);
				do {
					scanf("%d", &colla.persona[colla.numPersones-1].import);
					if (colla.persona[colla.numPersones-1].import < 5 || colla.persona[colla.numPersones-1].import > 60 || colla.persona[colla.numPersones-1].import%5!=0)
						printf("%s", linies[4]);
				} while (colla.persona[colla.numPersones-1].import < 5 || colla.persona[colla.numPersones-1].import > 60 || colla.persona[colla.numPersones-1].import%5!=0);

				colla.persona[colla.numPersones-1].premi = 0;
				break;
			...
			case 2:
				...
				do {
					printf("%s", linies[48]);
					scanf("%s", user);
					for (i = 0; i < colla.numPersones && !trobat; i++) {
						if (strcmp(user, colla.persona[i].nom) == 0) {
							trobat = 1;
							printf("%s", linies[49]);					
							do {
